...
function req(auth?: string) {
  return new NextRequest('http://localhost/api/cron/sync-prices',
...
test('no-ops green in multi-tenant mode', async () => {
  process.env.CRON_SECRET = 's3cret'
  process.env.TENANCY_MODE = 'multi'
  const res = await GET(req('Bearer s3cret'))
  assert.equal(res.status, 200)
  assert.deepEqual(await res.json(), { skipped: 'multi-tenant' })
})
