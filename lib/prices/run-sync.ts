...
interface RunSyncDeps {
  ...
  sweepLorcana?: typeof sweepLorcast
...
// YGO is a single cheap call, Lorcana ~20 gated set calls), then per-card
// in-stock sync, the stalest-first
...
export async function runFullPriceSync(db: Db, deps: RunSyncDeps = {}) {
  ...
  const lorcastSweep = await (deps.sweepLorcana ?? sweepLorcast)(settings, db)
